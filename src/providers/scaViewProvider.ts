...
// Using DetailedVulnerability, but expect SCA data to conform to ScaVulnerabilityWithCvssDto
import { ApiService, ScanType } from '../api/apiService';
import type { ProjectConfig } from '../auth/authService';
import { COMMAND_OPEN_FILE_LOCATION, COMMAND_SHOW_DETAILS } from '../constants/constants'; // Added COMMAND_OPEN_FILE_LOCATION
import { DetailedVulnerability, ScaVulnerabilityWithCvssDto } from '../dtos/result/details';

/**
 * Webview Provider for displaying SCA findings.
 * Loads findings on activation and handles clicks to show details and potentially open the manifest file.
 */
export class ScaViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
    public static readonly viewType = 'cybedefendScanner.scaView'; // Specific View Type
    private readonly scanType: ScanType = 'sca'; // Specific Scan Type
    ...
    /**
     * Creates an instance of ScaViewProvider.
    ...
    /**
     * Fetches SCA findings from the API.
     */
